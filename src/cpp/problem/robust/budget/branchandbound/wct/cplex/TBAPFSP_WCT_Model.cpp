...
namespace robust {
    namespace hybrid {
        ...
        *
        *   sum(P_bar[r - 1][i] * z[i][1] for i in 1:n)
          + sum(sum((P_bar[r][i] - P_bar[r-1][i]) * z[i][q] for i in 1:n) for q in 1:(j-1))
          + sum((x[r][s] - x[r-1][s]) for s in 2:j)
          - sum(P_bar[r-1][i] * z[i][j] for i in 1:n) >= 0               for r in 2:m, j in 2:n  (c11)
        *   x[1][j] == 0                                                 for j in 2:n            (c7)
        *   f[i] >= sum(sum(P_bar[p][x] * z[x][1] for x in 1:n) for p in 1:(m-1))
            + sum(sum(P_bar[m][x] * z[x][q] for x in 1:n) for q in 1:j)
            + sum(x[m][s] for s in 1:j) - bigM * (1 - z[i][j])           for i in 1:n, j in 1:n  (c14)
        */
        NumVarMatrix TBAPFSP_WCT_Model::populate_model(IloCplex &cplex, IloModel &model, IloRangeArray &c,
                                                IloNumVar &obj,
            ...
            for(int cut_count = 0; cut_count < cut_list.size(); cut_count++) {
                ...
                for (IloInt r = 2 - 1; r < m; r++) {
                    for (IloInt j = 2 - 1; j < n; j++) {
                        ...
                        //  + sum(sum((P_bar[r][i] - P_bar[r-1][i]) * z[i][q] for i in 1:n) for q in 1:(j-1))
                        for (IloInt q = 0; q <= j - 1; q++) {
                            for (IloInt i = 0; i < n; i++) {
                                expr1 += (P_bar(r+1, i+1) + P_hat(r+1, i+1) * sep(r+1, i+1)) * z[i][q];
                                expr1 += -(P_bar(r-1+1, i+1) + P_hat(r-1+1, i+1) * sep(r-1+1, i+1)) * z[i][q];
                            }
                        }
                        //  + sum((x[r][s] - x[r-1][s]) for s in 2:j)
                        for (IloInt s = 2 - 1; s <= j; s++) {
                            expr1 += x[r][s] - x[r-1][s];
                        }
                        //  - sum(P_bar[r-1][i] * z[i][j] for i in 1:n)
                        for (IloInt i = 0; i < n; i++) {
                ...
                // (c7) x[1][j] == 0 , for j in 2:n
                for (IloInt j = 2 - 1; j < n; j++) {
                    model.add(x[1 - 1][j] == 0);
                }
                // (c14) Disjuctive constraints used to determine the start time of job i on the last machine m
                for (IloInt i = 0; i < n; i++) {
                    for (IloInt j = 0; j < n; j++) {
                        IloExpr expr1(env);
                        // sum(sum(P_bar[p, x] * Z[x, 1] for x in 1:n) for p in 1:(m-1))
                        for (IloInt p = 0; p < m - 1; p++) {
                            for (IloInt x = 0; x < n; x++) {
                                expr1 += (P_bar(p+1, x+1) + P_hat(p+1, x+1) * sep(p+1, x+1)) * z[x][1-1];
                            }
                        }
                        //    + sum(sum(P_bar[m, x] * Z[x, q] for x in 1:n) for q in 1:j)
                        for (IloInt q = 0; q <= j; q++) {
                            for (IloInt x = 0; x < n; x++) {
                                expr1 += (P_bar(m, x+1) + P_hat(m, x+1) * sep(m, x+1)) * z[x][q];
                            }
                        }
                        //    + sum(X[m, s] for s in 1:j)
                        for (IloInt s = 0; s <= j; s++) {
                            expr1 += x[m-1][s];
                        }
                        // F[i] >= expr1 - bigM * (1 - Z[i, j]);
        ...
        void TBAPFSP_WCT_Model::fix_partial_permutation_in_model (IloModel &model, NumVarMatrix &z, IloRangeArray &c,
         ...
         */
        ublas::matrix<double> TBAPFSP_WCT_Model::calculate_cmax_given_scenario(int m, int n, const ublas::matrix<double> &P_bar,
         ...
         * @return bigM value.
         */
        double TBAPFSP_WCT_Model::calculate_bigM(int m, int n, const ublas::matrix<double> &P_bar,
                                                const ublas::matrix<double> &P_hat,
         ...
         */
        void TBAPFSP_WCT_Model::set_warm_start_solution(IloCplex &cplex, IloModel &model, const int &n,
            ...
            startVal.end();
            startVar.end();
            LOG(INFO) << "TBA PFSP-WCT MP warm start solution set.";
        }

        std::vector<int> TBAPFSP_WCT_Model::getSolutionAsPermutation(IloCplex &cplex, IloModel &model,
