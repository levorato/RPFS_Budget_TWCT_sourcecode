...
namespace robust {
    namespace hybrid {
        ...
        *   e[i][k] = completion time of job k on machine i
        ...
        *
        *   e[r][j+1] >= e[r][j] + sum(T[r][i] * z[i][j+1] for i in Jobs)       for r in 2:m, j in 1:(n-1)   (A.17)
        *   e[r+1][j] >= e[r][j] + sum(T[r+1][i] * z[i][j] for i in Jobs)       for r in 1:(m-1), j in 2:n   (A.18)
        *   e[1][j+1] == e[1][j] + sum(T[1][i] * z[i][j+1] for i in Jobs)       for j in 1:(n-1)             (A.19)
        *   e[r+1][1] == e[r][1] + sum(T[r+1][i] * z[i][1] for i in Jobs)       for r in 1:(m-1)             (A.20)
        *   e[1][1] == sum(T[1][i] * z[i][1] for i in Jobs)                                                  (A.21)
        *   f[i] >= e[m][j] - bigM * (1 - z[i][j])                              for i in 1:n, j in 1:n
        */
        NumVarMatrix TS2PFSP_WCT_Model::populate_model(IloCplex &cplex, IloModel &model, IloRangeArray &c,
                                                IloNumVar &obj,
            ...
            for(int cut_count = 0; cut_count < cut_list.size(); cut_count++) {
                ...
                for (IloInt i = 0; i < m; i++) {
                    E[i] = IloNumVarArray(env, n, 0, IloInfinity);
                    std::stringstream se;
                    se << "E_" << cut_count << "(" << i << ")";
                    E[i].setNames(se.str().c_str());
                }
                for (IloInt i = 0; i < n; i++) {
                    std::stringstream sb;
                    sb << "F_" << cut_count << "(" << i << ")";
                ...
                // add the constraints
                // (A.17) e[r][j+1] >= e[r][j] + sum(T[r][i] * z[i][j+1] for i in Jobs), for r in 2:m, j in 1:(n-1)
                for (IloInt r = 2-1; r < m; r++) {
                    for (IloInt j = 0; j < n - 1; j++) {
                        ...
                        model.add(E[r][j+1] >= expr1);
                        expr1.end();
                    }
                }
                // (A.18) e[r+1][j] >= e[r][j] + sum(T[r+1][i] * z[i][j] for i in Jobs), for r in 1:(m-1), j in 2:n
                for (IloInt r = 0; r < m - 1; r++) {
                    for (IloInt j = 2-1; j < n; j++) {
                        ...
                        model.add(E[r+1][j] >= expr1);
                        expr1.end();
                    }
                }
                // (A.19) e[1][j+1] == e[1][j] + sum(T[1][i] * z[i][j+1] for i in Jobs), for j in 1:(n-1)
                for (IloInt j = 0; j < n - 1; j++) {
                    IloExpr expr1(env);
                    expr1 += E[1-1][j];
                    for (IloInt i = 0; i < n; i++) {
                    ...
                    model.add(E[1-1][j+1] == expr1);
                    expr1.end();
                }
                // (A.20) e[r+1][1] == e[r][1] + sum(T[r+1][i] * z[i][1] for i in Jobs), for r in 1:(m-1)
                for (IloInt r = 0; r < m - 1; r++) {
                    IloExpr expr1(env);
                    expr1 += E[r][1-1];
                    for (IloInt i = 0; i < n; i++) {
                    ...
                    model.add(E[r+1][1-1] == expr1);
                    expr1.end();
                }
                // (A.21) e[1][1] == sum(T[1][i] * z[i][1] for i in Jobs)
                IloExpr expr2(env);
                for (IloInt i = 0; i < n; i++) {
                    expr2 += (P_bar(1, i+1) + P_hat(1, i+1) * sep(1, i+1)) * z[i][1-1];
                }
                model.add(E[1-1][1-1] == expr2);
                // Disjuctive constraints used to determine the start time of job i on the last machine m
                for (IloInt i = 0; i < n; i++) {
                    for (IloInt j = 0; j < n; j++) {
                        model.add(F[i] >= E[m-1][j] - bigM * (1 - z[i][j]));
                    }
                }
        ...
        void TS2PFSP_WCT_Model::fix_partial_permutation_in_model (IloModel &model, NumVarMatrix &z, IloRangeArray &c,
         ...
         */
        ublas::matrix<double> TS2PFSP_WCT_Model::calculate_cmax_given_scenario(int m, int n, const ublas::matrix<double> &P_bar,
         ...
         * @return bigM value.
         */
        double TS2PFSP_WCT_Model::calculate_bigM(int m, int n, const ublas::matrix<double> &P_bar,
                                                const ublas::matrix<double> &P_hat,
         ...
         */
        void TS2PFSP_WCT_Model::set_warm_start_solution(IloCplex &cplex, IloModel &model, const int &n,
            ...
            startVal.end();
            startVar.end();
            LOG(INFO) << "TS2 PFSP-WCT MP warm start solution set.";
        }

        std::vector<int> TS2PFSP_WCT_Model::getSolutionAsPermutation(IloCplex &cplex, IloModel &model,
