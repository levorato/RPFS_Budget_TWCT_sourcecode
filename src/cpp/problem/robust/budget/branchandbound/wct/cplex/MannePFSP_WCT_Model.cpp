...
namespace robust {
    namespace hybrid {
        ...
        *   C[r][i] >= 0 : completion time of job i on machine r
        *   obj = objective function variable
        *
        * minimize obj
        * s.t.
        *   sum(w[i] * C[m, i] for i in Jobs) <= obj
        *
        *   C[1, i] >= T[1, i]                                                for i in 1..n
        *   C[r, i] - C[r-1, i] >= T[r, i]                                    for r in 2..m, i in 1..n
        *   C[r, i] - C[r, k] + Q * D[i, k] >= T[r, i]                        for r in 1..m, i in 1..n-1, k in i+1..n
        *   C[r, i] - C[r, k] + Q * D[i, k] <= Q - T[r, k]                    for r in 1..m, i in 1..n-1, k in i+1..n
        */
        NumVarMatrix MannePFSP_WCT_Model::populate_model(IloCplex &cplex, IloModel &model, IloRangeArray &c, 
                                                IloNumVar &obj,
            ...
            for(int cut_count = 0; cut_count < cut_list.size(); cut_count++) {
                ...
                // C[r, i] >= 0 : completion time of job i on machine r => [M, N]
                NumVarMatrix C(env, m);  
                for (IloInt r = 0; r < m; r++) {
                    C[r] = IloNumVarArray(env, n, 0, IloInfinity);  // C[r, i] >= 0
                    std::stringstream sc;
                    sc << "C_" << cut_count << "(" << r << ")";
                    C[r].setNames(sc.str().c_str());
                }
                // add the constraints
                // (1) C[1, i] >= T[1, i], for i in 1..n
                for (IloInt i = 0; i < n; i++) {
                    model.add(C[0][i] >= P_bar(1, i+1) + (P_hat(1, i+1) * sep(1, i+1)));
                }
                // (2) C[r, i] - C[r-1, i] >= T[r, i], for r in 2..m, i in 1..n
                for (IloInt r = 1; r < m; r++) {
                    for (IloInt i = 0; i < n; i++) {
                        IloExpr expr1(env);  // (2)
                        expr1 += C[r][i] - C[r-1][i];
                        model.add(expr1 >= P_bar(r+1, i+1) + (P_hat(r+1, i+1) * sep(r+1, i+1)));
                        expr1.end();
                    }
                }
                // (3) C[r, i] - C[r, k] + Q * D[i, k] >= T[r, i], for r in 1..m, i in 1..n-1, k in i+1..n
                // (4) C[r, i] - C[r, k] + Q * D[i, k] <= Q - T[r, k], for r in 1..m, i in 1..n-1, k in i+1..n
                for (IloInt r = 0; r < m; r++) {
                    for (IloInt i = 0; i < n - 1; i++) {
                        for (IloInt k = i + 1; k < n; k++) {
                            IloExpr expr1(env);  // (3)
                            IloExpr expr2(env);  // (3)
                            expr1 += C[r][i] - C[r][k] + bigM * d[i][k];
                            expr2 += P_bar(r+1, i+1) + (P_hat(r+1, i+1) * sep(r+1, i+1));
                            model.add(expr1 >= expr2);
                            expr1.end();
                            expr2.end();
                            IloExpr expr3(env);  // (4)
                            IloExpr expr4(env);  // (4)
                            expr3 += C[r][i] - C[r][k] + bigM * d[i][k];
                            expr4 += bigM - (P_bar(r+1, k+1) + (P_hat(r+1, k+1) * sep(r+1, k+1)));
                ...
                // (4) sum(w[i] * C[m, i] for i in Jobs) <= obj
                IloExpr expr0(env);
                for (IloInt i = 0; i < n; i++) {
                    expr0 += w(i + 1) * C[m - 1][i];
                }
        ...
        void MannePFSP_WCT_Model::fix_partial_permutation_in_model (IloModel &model, NumVarMatrix &d, IloRangeArray &c,
         ...
         */
        ublas::matrix<double> MannePFSP_WCT_Model::calculate_cmax_given_scenario(int m, int n, const ublas::matrix<double> &P_bar,
         ...
         * @return bigM value.
         */
        double MannePFSP_WCT_Model::calculate_bigM(int m, int n, const ublas::matrix<double> &P_bar,
                                                const ublas::matrix<double> &P_hat,
         ...
         */
        void MannePFSP_WCT_Model::set_warm_start_solution(IloCplex &cplex, IloModel &model, const int &n, 
            ...
            startVal.end();
            startVar.end();
            LOG(INFO) << "Manne PFSP-WCT MP warm start solution set.";
        }

        std::vector<int> MannePFSP_WCT_Model::getSolutionAsPermutation(IloCplex &cplex, IloModel &model, 
         ...
         */
        std::vector<int> MannePFSP_WCT_Model::topological_sort_by_dfs(const int &n, const ublas::matrix<int> &d_matrix) {
        ...
        std::vector<int> MannePFSP_WCT_Model::topological_sort_by_dfs_v2(const int &nv, const ublas::matrix<int> &g) {
            // graph `g` as a vector of vertices in topological order.
