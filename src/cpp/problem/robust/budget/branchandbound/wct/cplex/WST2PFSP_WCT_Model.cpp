...
namespace robust {
    namespace hybrid {
        ...
        *   x[r][j] = idle time on machine r before the start of job in sequence position j
        ...
        *
        *   sum(T[r][i] * z[i][j+1] for i in Jobs)
            - sum(T[r+1][i] * z[i][j] for i in Jobs) + x[r][j+1] - x[r+1][j+1] + y[r][j+1] - y[r][j] == 0
                                                             for r in 2:m-1, for j in 2:n-1                 (A.1)
        *   sum(T[1][i] * z[i][j+1] for i in Jobs) - sum(T[2][i] * z[i][j] for i in Jobs)
            - x[2][j+1] + y[1][j+1] - y[1][j] == 0           for j in 2:n-1                                 (A.2)
        *   sum(T[r][i] * z[i][2] for i in Jobs) - sum(T[r+1][i] * z[i][1] for i in Jobs)
            + x[r][2] - x[r+1][2] + y[r][2] == 0             for r in 2:m-1                                 (A.3)
        *   sum(T[1][i] * z[i][2] for i in Jobs) - sum(T[2][i] * z[i][1] for i in Jobs)
            + y[1][2] - x[2][2] == 0                                                                        (A.4)
        *   sum(T[r][i] * z[i][1] for i in Jobs) + x[r][1] - x[r+1][1] == 0
                                                             for r in 1:m-1                                 (A.5)
        *   x[1][1] == 0                                                                                    (A.6)
        *   f[i] >= sum(T[m][x] * z[x][p] for p in 1:j, x in 1:n) + sum(x[m][p] for p in 1:j)
            - bigM * (1 - z[i][j])                           for i in 1:n, for j in 1:n
        */
        NumVarMatrix WST2PFSP_WCT_Model::populate_model(IloCplex &cplex, IloModel &model, IloRangeArray &c,
                                                IloNumVar &obj,
            ...
            for(int cut_count = 0; cut_count < cut_list.size(); cut_count++) {
                ...
                for (IloInt r = 0; r < m; r++) {
                    x[r] = IloNumVarArray(env, n, 0, IloInfinity);
                    std::stringstream sx;
                    sx << "x_" << cut_count << "(" << r << ")";
                    x[r].setNames(sx.str().c_str());
                ...
                // add the constraints
                // (A.1) for r in 2:m-1, for j in 2:n-1 :
                //  sum(T[r][i] * z[i][j+1] for i in Jobs)
                //    - sum(T[r+1][i] * z[i][j] for i in Jobs) + x[r][j+1] - x[r+1][j+1] + y[r][j+1] - y[r][j] == 0
                for (IloInt r = 2 - 1; r < m - 1; r++) {
                    for (IloInt j = 2 - 1; j < n - 1; j++) {
                        IloExpr expr1(env);
                        // + x[r][j+1] - x[r+1][j+1] + y[r][j+1] - y[r][j]
                        expr1 += x[r][j+1] - x[r+1][j+1] + y[r][j+1] - y[r][j];
                        //  sum(T[r][i] * z[i][j+1] for i in Jobs) - sum(T[r+1][i] * z[i][j] for i in Jobs)
                        for (IloInt i = 0; i < n; i++) {
                            expr1 += (P_bar(r+1, i+1) + P_hat(r+1, i+1) * sep(r+1, i+1)) * z[i][j+1];
                            expr1 += -(P_bar(r+1+1, i+1) + P_hat(r+1+1, i+1) * sep(r+1+1, i+1)) * z[i][j];
                        }
                        model.add(expr1 == 0);
                        expr1.end();
                    }
                ...
                // (A.2) for j in 2:n-1 :
                // sum(T[1][i] * z[i][j+1] for i in Jobs) - sum(T[2][i] * z[i][j] for i in Jobs)
                //    - x[2][j+1] + y[1][j+1] - y[1][j] == 0
                for (IloInt j = 2 - 1; j < n - 1; j++) {
                    IloExpr expr1(env);
                    // - x[2][j+1] + y[1][j+1] - y[1][j]
                    expr1 += - x[2-1][j+1] + y[1-1][j+1] - y[1-1][j];
                    // sum(T[1][i] * z[i][j+1] for i in Jobs) - sum(T[2][i] * z[i][j] for i in Jobs)
                    for (IloInt i = 0; i < n; i++) {
                        expr1 += (P_bar(1, i+1) + P_hat(1, i+1) * sep(1, i+1) ) * z[i][j+1];
                        expr1 += -(P_bar(2, i+1) + P_hat(2, i+1) * sep(2, i+1) ) * z[i][j];
                    }
                    model.add(expr1 == 0);
                    expr1.end();
                }
                // (A.3) for r in 2:m-1 :
                // sum(T[r][i] * z[i][2] for i in Jobs) - sum(T[r+1][i] * z[i][1] for i in Jobs)
                //    + x[r][2] - x[r+1][2] + y[r][2] == 0
                for (IloInt r = 2 - 1; r < m - 1; r++) {
                    for (IloInt j = 1; j < n; j++) {
                        IloExpr expr1(env);
                        // x[r][2] - x[r+1][2] + y[r][2]
                        expr1 += x[r][2-1] - x[r+1][2-1] + y[r][2-1];
                        // sum(T[r][i] * z[i][2] for i in Jobs) - sum(T[r+1][i] * z[i][1] for i in Jobs)
                        for (IloInt i = 0; i < n; i++) {
                            expr1 += (P_bar(r+1, i+1) + P_hat(r+1, i+1) * sep(r+1, i+1)) * z[i][2-1];
                            expr1 += -(P_bar(r+1+1, i+1) + P_hat(r+1+1, i+1) * sep(r+1+1, i+1)) * z[i][1-1];
                        }
                        model.add(expr1 == 0);
                        expr1.end();
                    }
                ...
                IloExpr expr2(env);
                // + y[1][2] - x[2][2]
                expr2 += y[1-1][2-1] - x[2-1][2-1];
                // sum(T[1][i] * z[i][2] for i in Jobs) - sum(T[2][i] * z[i][1] for i in Jobs)
                for (IloInt i = 0; i < n; i++) {
                    expr2 += (P_bar(1, i+1) + P_hat(1, i+1) * sep(1, i+1)) * z[i][2-1];
                    expr2 += -(P_bar(1+1, i+1) + P_hat(1+1, i+1) * sep(1+1, i+1)) * z[i][1-1];
                }
                model.add(expr2 == 0);
                expr2.end();
                // (A.5) for r in 1:m-1 :
                ...
                for (IloInt r = 0; r < m - 1; r++) {
                    IloExpr expr1(env);
                    // + x[r][1] - x[r+1][1]
                    expr1 += x[r][1-1] - x[r+1][1-1];
                    // sum(T[r][i] * z[i][1] for i in Jobs)
                    for (IloInt i = 0; i < n; i++) {
                        expr1 += (P_bar(r+1, i+1) + P_hat(r+1, i+1) * sep(r+1, i+1)) * z[i][1-1];
                    }
                    model.add(expr1 == 0);
                    expr1.end();
                }
                // (A.6) x[1][1] == 0
                model.add(x[1-1][1-1] == 0);
                ...
                for (IloInt i = 0; i < n; i++) {
                    for (IloInt j = 0; j < n; j++) {
                        IloExpr expr1(env);
                        // sum(T[m][x] * z[x][p] for p in 1:j, x in 1:n)
                        for (IloInt p = 0; p <= j; p++) {
                            for (IloInt x = 0; x < n; x++) {
                                expr1 += (P_bar(m, x+1) + P_hat(m, x+1) * sep(m, x+1)) * z[x][p];
                            }
                        }
                        // sum(x[m][p] for p in 1:j)
                        for (IloInt p = 0; p <= j; p++) {
                            expr1 += x[m-1][p];
                        }
        ...
        void WST2PFSP_WCT_Model::fix_partial_permutation_in_model (IloModel &model, NumVarMatrix &z, IloRangeArray &c,
         ...
         */
        ublas::matrix<double> WST2PFSP_WCT_Model::calculate_cmax_given_scenario(int m, int n, const ublas::matrix<double> &P_bar,
         ...
         * @return bigM value.
         */
        double WST2PFSP_WCT_Model::calculate_bigM(int m, int n, const ublas::matrix<double> &P_bar,
                                                const ublas::matrix<double> &P_hat,
         ...
         */
        void WST2PFSP_WCT_Model::set_warm_start_solution(IloCplex &cplex, IloModel &model, const int &n,
            ...
            startVal.end();
            startVar.end();
            LOG(INFO) << "WST2 PFSP-WCT MP warm start solution set.";
        }

        std::vector<int> WST2PFSP_WCT_Model::getSolutionAsPermutation(IloCplex &cplex, IloModel &model,
