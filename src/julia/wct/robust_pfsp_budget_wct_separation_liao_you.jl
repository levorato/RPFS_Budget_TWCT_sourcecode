# ===================================================================================================
# robust_pfsp_budget_wct_separation_liao_you.jl
...
# based on Liao-You PFSP MILP Model.
# ===================================================================================================
...
function separate_liao_you(model, m, n, Γ, P_bar, P_hat, w, iteration, UB, best_pi, bigM; max_cores = 16,
    ...
    for permutation in Set(permutation_current)  # Process all initial solutions (or current CCG solution)
        ...
        if gap > EPS  # If Gap = (UB - LB) / LB <= EPS, an eps-optimal solution is found, terminate.
            ...
            for z_sep in z_sep_list
                ...
                new_S = @variable(model, [M, N], base_name = "new_S_$(iteration)-$(count)", lower_bound = 0)
                # Surplus variables q[r, i, k]
                new_q = @variable(model, [M, N, N], base_name = "new_q_$(iteration)-$(count)", lower_bound = 0)

                # New objective function constraint
                @constraint(model, sum(w[i] * (new_S[m, i] + P_bar[m, i] + (P_hat[m, i]*z_sep[m, i])) for i in Jobs) <= obj)
                # New problem constraints for this specific scenario z_sep
                for r in 1:(m-1)
                    for i in 1:n
                        @constraint(model, new_S[r, i] + P_bar[r, i] + (P_hat[r, i]*z_sep[r, i]) <= new_S[r+1, i])
                    end
                end
                for r in 1:m
                    for i in 1:(n-1)
                        for k in (i+1):n
                            @constraint(model, new_S[r, i] - new_S[r, k] + P * D[i, k] - (P_bar[r, k] + (P_hat[r, k]*z_sep[r, k])) == new_q[r, i, k])
                            @constraint(model, P - (P_bar[r, i] + P_hat[r, i]*z_sep[r, i]) - (P_bar[r, k] + (P_hat[r, k]*z_sep[r, k])) >= new_q[r, i, k])
                        end
                    end
                end
            end
            println("Added $(count) cuts to MP.")
            total_count += count
        else
...
# ===================================================================================
# Based on Liao-You PFSP formulation
function solve_robust_model_flowshop_wct_budget_separation_liao_you(m, n, P_bar, P_hat, w, Γ, time_limit; mp_bigM_type = 1, sp_bigM_type = 3, max_cores = 16,
    ...
    println("Starting Liao-You C&CG procedure...")
    ...
    ## ++++++++++ Model variables:
    # S_ri start time of job i on machine r
    S = @variable(model, [M, N], base_name = "new_S_0", lower_bound = 0)
    # Surplus variables q[r, i, k]
    q = @variable(model, [M, N, N], base_name = "new_q_0", lower_bound = 0)
    ...
    @constraint(model, sum(w[i] * (S[m, i] + T[m, i]) for i in Jobs) <= obj)
    iter = 1
    UB = typemax(Float64)
    LB = 0.0
    best_pi = []
    ...
    while true
        # Solve the problem
        println("====================================================================\n")
        println("Solving WCT budget robust Liao-You model C & CG (Iteration $(iter))...")
        println("====================================================================\n")
        ...
        if status == MOI.OPTIMAL
            ...
            if warm_start
                ...
                println("\n===========  L I A O - Y O U    R O B U S T    S O L U T I O N  ===========\n")
            ...
            separation_done, mip_optimal, UB, LB, best_pi, gap, sp_time, cut_count = separate_liao_you(model, m, n, Γ, P_bar, P_hat, w, iter + 1, UB, best_pi, bigM;
            ...
            if separation_done
                println(" *** Separation done! y_star = $(y_star), best_pi = $(best_pi), Best UB: $(UB) ; LB = $(LB)")
                flush(stdout)
