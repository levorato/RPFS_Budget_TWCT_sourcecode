# ===================================================================================================
# robust_pfsp_budget_wct_separation_ts3.jl
...
# based on TS3 PFSP MILP Model, from
...
function separate_ts3(model, m, n, Γ, P_bar, P_hat, w, iteration, UB, best_pi, bigM; max_cores = 16,
    ...
    for permutation in Set(permutation_current)  # Process all initial solutions (or current CCG solution)
        ...
        if gap > EPS  # If Gap = (UB - LB) / LB <= EPS, an eps-optimal solution is found, terminate.
            ...
            for z_sep in z_sep_list
                ...
                for r in 2:m
                    for j in 2:n
                        @constraint(model, sum(P_bar[1, i] * Z[i, j-1] for i in 1:n) + sum(P_hat[1, i] * Z[i, j-1] for i in 1:n if z_sep[1, i] == 1)
                            - sum(P_bar[r, i] * Z[i, j-1] for i in 1:n) - sum(P_hat[r, i] * Z[i, j-1] for i in 1:n if z_sep[r, i] == 1)
                            + sum(sum(P_bar[q, i] * (Z[i, j] - Z[i, j-1]) for i in 1:n) + sum(P_hat[q, i] * (Z[i, j] - Z[i, j-1]) for i in 1:n if z_sep[q, i] == 1) for q in 1:(r - 1))
                            + sum(newY[q, j] - newY[q, j-1] for q in 1:(r - 1)) >= 0)
                    end
                end
                # Constraints (20)
                for r in 1:(m - 1)
                    @constraint(model, newY[r, 1] == 0)
                end
                ...
                for i in 1:n  # Disjuctive constraints used to determine the flow time of job i on the last machine m
                    for j in 1:n
                        @constraint(model, newF[i] >= sum(sum(P_bar[1, x] * Z[x, p] for x in 1:n) + sum(P_hat[1, x] * Z[x, p] for x in 1:n if z_sep[1, x] == 1) for p in 1:j)
                            + sum(sum(P_bar[q, x] * Z[x, j] for x in 1:n) + sum(P_hat[q, x] * Z[x, j] for x in 1:n if z_sep[q, x] == 1) for q in 2:m)
                            + sum(newY[q, j] for q in 1:(m - 1)) - bigM * (1 - Z[i, j]) )
                    end
                end
            end
            total_count += count
...
# ===================================================================================
function solve_robust_model_flowshop_wct_budget_separation_ts3(m, n, P_bar, P_hat, w, Γ, time_limit; mp_bigM_type = 2, sp_bigM_type = 3, max_cores = 16,
    ...
    bigM = max_cmax  # Big-M
    println("Starting TS3 C&CG procedure...")
    ...
    for r in 2:m
        for j in 2:n
            @constraint(model, sum(P_bar[1, i] * Z[i, j-1] for i in 1:n) - sum(P_bar[r, i] * Z[i, j-1] for i in 1:n)
    ...
    #    @constraint(model, F[j] == sum(TT[1, p] for p in 1:j) + sum(TT[q, j] for q in 2:m) + sum(Y[q, j] for q in 1:(m - 1)))
    #end
    # TT[r, j] == sum(T[r, i] * Z[i, j] for i in 1:n)
    for i in 1:n  # Disjuctive constraints used to determine the flow time of job i on the last machine m
        for j in 1:n
            @constraint(model, F[i] >= sum(sum(P_bar[1, x] * Z[x, p] for x in 1:n) for p in 1:j)
    ...
    while true
        # Solve the problem
        println("====================================================================\n")
        println("Solving WCT budget robust TS3 model C & CG (Iteration $(iter))...")
        println("====================================================================\n")
        ...
        if status == MOI.OPTIMAL
            ...
            if warm_start
                ...
                println("\n===========  T S 3    R O B U S T    S O L U T I O N  ===========\n")
            ...
            separation_done, mip_optimal, UB, LB, best_pi, gap, sp_time, cut_count = separate_ts3(model, m, n, Γ, P_bar, P_hat, w, iter + 1, UB, best_pi, bigM;
