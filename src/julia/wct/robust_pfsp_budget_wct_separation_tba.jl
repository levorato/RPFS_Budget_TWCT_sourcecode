# ===================================================================================================
# robust_pfsp_budget_wct_separation_tba.jl
...
# based on TBA PFSP MILP Model, presented in:
...
function separate_tba(model, m, n, Γ, P_bar, P_hat, w, iteration, UB, best_pi, bigM; max_cores = 16,
    ...
    for permutation in Set(permutation_current)  # Process all initial solutions (or current CCG solution)
        ...
        if gap > EPS  # If Gap = (UB - LB) / LB <= EPS, an eps-optimal solution is found, terminate.
            ...
            for z_sep in z_sep_list
                ...
                for r in 2:m
                    for j in 2:n
                        @constraint(model, sum(P_bar[r - 1, i] * Z[i, 1] for i in 1:n)
                          + sum(P_hat[r - 1, i] * Z[i, 1] for i in 1:n if z_sep[r - 1, i] == 1)
                          + sum(sum((P_bar[r, i] - P_bar[r-1, i]) * Z[i, q] for i in 1:n) for q in 1:(j-1))
                          + sum(sum(P_hat[r, i] * Z[i, q] for i in 1:n if z_sep[r, i] == 1) for q in 1:(j-1))
                          - sum(sum(P_hat[r-1, i] * Z[i, q] for i in 1:n if z_sep[r-1, i] == 1) for q in 1:(j-1))
                          + sum((newX[r, s] - newX[r-1, s]) for s in 2:j)
                          - sum(P_bar[r-1, i] * Z[i, j] for i in 1:n)
                          - sum(P_hat[r-1, i] * Z[i, j] for i in 1:n if z_sep[r-1, i] == 1) >= 0)
                    end
                end
                for j in 2:n  # Constraints (7)
                    @constraint(model, newX[1, j] == 0)
                end
                ...
                #end
                for i in 1:n  # Disjuctive constraints used to determine the flow time of job i on the last machine m
                    for j in 1:n
                        @constraint(model, newF[i] >= sum(sum(P_bar[p, x] * Z[x, 1] for x in 1:n) for p in 1:(m-1))
                            + sum(sum(P_hat[p, x] * Z[x, 1] for x in 1:n if z_sep[p, x] == 1) for p in 1:(m-1))
                            + sum(sum(P_bar[m, x] * Z[x, q] for x in 1:n) for q in 1:j)
                            + sum(sum(P_hat[m, x] * Z[x, q] for x in 1:n if z_sep[m, x] == 1) for q in 1:j)
                            + sum(newX[m, s] for s in 1:j) - bigM * (1 - Z[i, j]) )
                    end
                end
            end
            total_count += count
...
# ===================================================================================
function solve_robust_model_flowshop_wct_budget_separation_tba(m, n, P_bar, P_hat, w, Γ, time_limit; mp_bigM_type = 2, sp_bigM_type = 3, max_cores = 16,
    ...
    bigM = max_cmax  # Big-M
    println("Starting TBA C&CG procedure...")
    ...
    # Constraints (11)Wagner
    for r in 2:m
        for j in 2:n
            @constraint(model, sum(P_bar[r - 1, i] * Z[i, 1] for i in 1:n)
              + sum(sum((P_bar[r, i] - P_bar[r-1, i]) * Z[i, q] for i in 1:n) for q in 1:(j-1))
              + sum((X[r, s] - X[r-1, s]) for s in 2:j)
              - sum(P_bar[r-1, i] * Z[i, j] for i in 1:n) >= 0)
        end
    end
    for j in 2:n  # Constraints (7)
    ...
    for i in 1:n  # Disjuctive constraints used to determine the flow time of job i on the last machine m
        for j in 1:n
            @constraint(model, F[i] >= sum(sum(P_bar[p, x] * Z[x, 1] for x in 1:n) for p in 1:(m-1))
                + sum(sum(P_bar[m, x] * Z[x, q] for x in 1:n) for q in 1:j)
                + sum(X[m, s] for s in 1:j) - bigM * (1 - Z[i, j]) )
        end
    end
    ...
    while true
        # Solve the problem
        println("====================================================================\n")
        println("Solving WCT budget robust TBA model C & CG (Iteration $(iter))...")
        println("====================================================================\n")
        ...
        if status == MOI.OPTIMAL
            ...
            if warm_start
                ...
                println("\n===========  T B A    R O B U S T    S O L U T I O N  ===========\n")
            ...
            separation_done, mip_optimal, UB, LB, best_pi, gap, sp_time, cut_count = separate_tba(model, m, n, Γ, P_bar, P_hat, w, iter + 1, UB, best_pi, bigM;
