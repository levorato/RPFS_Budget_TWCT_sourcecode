# ===================================================================================================
# robust_pfsp_budget_wct_separation_wilson.jl
...
# based on Wilson PFSP MILP Model.
# ===================================================================================================
...
function separate_wilson(model, m, n, Γ, P_bar, P_hat, w, iteration, UB, best_pi, bigM; max_cores = 16,
    ...
    for permutation in Set(permutation_current)  # Process all initial solutions (or current CCG solution)
        ...
        if gap > EPS  # If Gap = (UB - LB) / LB <= EPS, an eps-optimal solution is found, terminate.
            ...
            for z_sep in z_sep_list
                ...
                newB = @variable(model, [M, N], base_name = "newB_$(iteration)-$(count)", lower_bound = 0)
                # E[i] : start time of job i on the last machine (M_m)
                newE = @variable(model, [N], base_name = "newE_$(iteration)-$(count)", lower_bound = 0)
                # Delta: auxiliary variable used in the BigM-based disjuction (see last constraint)
                ###newDelta = @variable(model, [N, N], base_name = "newDelta_$(iteration)-$(count)", Bin)

                # New objective function constraint
                @constraint(model, sum(w[i] * newE[i] for i in Jobs) + sum(w[i] * sum(P_bar[m, i] * Z[i, j] for j in Jobs) for i in Jobs) + sum(w[i] * sum(P_hat[m, i] * Z[i, j] for j in Jobs) for i in Jobs if z_sep[m, i] == 1) <= obj)
                #@constraint(model, newB[n] + sum(P_bar[2, i] * x[i, n] for i in N) + sum(P_hat[2, i] * x[i, n] for i in N if z_sep[2, i] == 1) <= obj)

                # New problem constraints for this specific scenario z_sep
                for j in 1:n-1
                    @constraint(model, newB[1, j] + sum(P_bar[1, i] * Z[i, j] for i in Jobs) + sum(P_hat[1, i] * Z[i, j] for i in Jobs if z_sep[1, i] == 1) == newB[1, j+1])
                end
                @constraint(model, newB[1, 1] == 0)
                for r in 1:m-1
                    @constraint(model, newB[r, 1] + sum(P_bar[r, i] * Z[i, 1] for i in Jobs) + sum(P_hat[r, i] * Z[i, 1] for i in Jobs if z_sep[r, i] == 1) == newB[r+1, 1])
                end
                for r in 1:m-1
                    for j in 2:n
                        @constraint(model, newB[r, j] + sum(P_bar[r, i] * Z[i, j] for i in Jobs) + sum(P_hat[r, i] * Z[i, j] for i in Jobs if z_sep[r, i] == 1) <= newB[r+1, j])
                    end
                end
                for r in 2:m
                    for j in 1:n-1
                        @constraint(model, newB[r, j] + sum(P_bar[r, i] * Z[i, j] for i in Jobs) + sum(P_hat[r, i] * Z[i, j] for i in Jobs if z_sep[r, i] == 1) <= newB[r, j+1])
                    end
                end
                for i in 1:n  # Disjuctive constraints used to determine the start time of job i on the last machine m
                    for j in 1:n
                        @constraint(model, newE[i] >= newB[m, j] - bigM * (1 - Z[i, j]) )
                        ###@constraint(model, newDelta[i, j] <= Z[i, j])
                    end
                    ###@constraint(model, sum(newDelta[i, j] for j in 1:n) == 1)
                end
            end
            total_count += count
        else
...
# ===================================================================================
function solve_robust_model_flowshop_wct_budget_separation_wilson(m, n, P_bar, P_hat, w, Γ, time_limit; mp_bigM_type = 2, sp_bigM_type = 3, max_cores = 16,
    ...
    bigM = max_cmax  # Big-M
    println("Starting Wilson C&CG procedure...")
    ...
    # B_rj : start (begin) time of job in sequence position j on machine r
    B = @variable(model, [M, N], base_name = "newB_1", lower_bound = 0)
    # E[i] : start time of job i on the last machine (M_m)
    E = @variable(model, [N], base_name = "newE_1", lower_bound = 0)
    # Delta: auxiliary variable used in the BigM-based disjuction (see last constraint)
    ...
    for j in 1:n-1
        @constraint(model, B[1, j] + sum(P_bar[1, i] * Z[i, j] for i in Jobs) == B[1, j+1])
    end
    @constraint(model, B[1, 1] == 0)
    for r in 1:m-1
        @constraint(model, B[r, 1] + sum(P_bar[r, i] * Z[i, 1] for i in Jobs) == B[r+1, 1])
    end
    for r in 1:m-1
        for j in 2:n
            @constraint(model, B[r, j] + sum(P_bar[r, i] * Z[i, j] for i in Jobs) <= B[r+1, j])
        end
    end
    for r in 2:m
        for j in 1:n-1
            @constraint(model, B[r, j] + sum(P_bar[r, i] * Z[i, j] for i in Jobs) <= B[r, j+1])
        end
    end
    ...
    for i in 1:n  # Disjuctive constraints used to determine the start time of job i on the last machine m
        for j in 1:n
            @constraint(model, E[i] >= B[m, j] - bigM * (1 - Z[i, j]) )
            ###@constraint(model, Delta[i, j] <= Z[i, j])
        end
        ###@constraint(model, sum(Delta[i, j] for j in 1:n) == 1)
    end

    # ++++++++++ Objective function  (Constraint (4))
    @variable(model, obj >= 0)  # Makespan
    @objective(model, Min, obj)
    @constraint(model, sum(w[i] * E[i] for i in Jobs) + sum(w[i] * sum(P_bar[m, i] * Z[i, j] for j in Jobs) for i in Jobs) <= obj)
    iter = 1
    UB = typemax(Float64)
    best_pi = []
    LB = 0.0
    ...
    while true
        # Solve the problem
        println("====================================================================\n")
        println("Solving WCT budget robust Wilson model C & CG (Iteration $(iter))...")
        println("====================================================================\n")
        ...
        if status == MOI.OPTIMAL
            ...
            if warm_start
                ...
                println("\n===========  W I L S O N    R O B U S T    S O L U T I O N  ===========\n")
            ...
            separation_done, mip_optimal, UB, LB, best_pi, gap, sp_time, cut_count = separate_wilson(model, m, n, Γ, P_bar, P_hat, w, iter + 1, UB, best_pi, bigM;
