...
function f!(pi, r, k, C, deviated_jobs, m, n, P_bar, P_hat, w, verbose = false)
    ...
    if r == 1
        ...
        if x in deviated_jobs[1]
            if verbose
                println("Deviating job $(x): $(C[1, k]) + $(P_hat[1, x])")
            end
            C[1, k] += P_hat[1, x]
        end
    else
        ...
        if x in deviated_jobs[r]
            if verbose
                println("Deviating job $(x): $(C[r, k]) + $(P_hat[r, x])")
            end
            C[r, k] += P_hat[r, x]
        end
    end
...
function worst_case_wct_dp_m_machines(pi, m, n, Γ, P_bar, P_hat, w)
        ...
        for g in 0:(ceil_T1+1)  # was 0:n
            for d in 0:max_time_deviation+1
                ...
                if d - ptk < 0
                    ...
                    if (g - 1 >= 0)  ## && (dynmatrix[r, k - 1, g, d] < dynmatrix[r, k - 1, g - 1, d - ptk]) #&& !(jobk in deviated_jobs[r, k - 1, g - 1, d - ptk][r])
    ...
    for r in 2:m
        #for g1 in 0:Int64(ceil(Γ[r - 1]))
            g1 = Int64(ceil(Γ[r - 1]))
            max_wct = dynmatrix[r - 1, n, Int64(ceil(Γ[r - 1])), max_time_deviation+1]
            best_deviated_jobs = deviated_jobs[r - 1, n, Int64(ceil(Γ[r - 1])), max_time_deviation+1]
            best_C = C[r - 1, n, Int64(ceil(Γ[r - 1])), max_time_deviation+1]
            for d in 0:max_time_deviation
                if max_wct < dynmatrix[r - 1, n, Int64(ceil(Γ[r - 1])), d]
                    max_wct = dynmatrix[r - 1, n, Int64(ceil(Γ[r - 1])), d]
                    best_deviated_jobs = deviated_jobs[r - 1, n, Int64(ceil(Γ[r - 1])), d]
                    best_C = C[r - 1, n, Int64(ceil(Γ[r - 1])), d]
                end
            end
            ...
            for k in 1:n
                ...
                ptk = Int64(ceil(P_hat[r, jobk]))
                #println("*** jobk = $(jobk)")
                for g in 0:(ceil_Tr+1)  # was 0:n, should be 0:Γ[r]
                    for d in 0:max_time_deviation+1
                        if d - ptk < 0
                            if k > 1
                                deviated_jobs[r, k, g, d] = deepcopy(deviated_jobs[r, k - 1, g, d])
                                ...
                                deviated_jobs[r, k, g, d] = deepcopy(best_deviated_jobs)
                                C[r, k, g, d] = deepcopy(best_C)
                            end
    ...
    wct = calculate_wct_given_scenario(pi, m, n, P_bar, P_hat, w, worst_Γ_scenario_dev, true)
...
function worst_case_wct_dp_m_machines_2(pi, m, n, Γ, P_bar, P_hat, w, verbose = false)
    ...
    C = Array{Array{Float64, 2}}(undef, m, n, max_gamma+1, max_time_deviation+1)
    fill!(C, zeros(Float64, m, n))
    fill!(dynmatrix, 0)
    fill!(deviated_jobs, [ [] for i in 1:m ])
    gamma = max_gamma
    ...
    if verbose
        print("Matriz C (base, sem desvios): ")
    end
    wct_0, C_mtx = ff(pi, m, 1, 0, [ [] for i in 1:m ], m, n, P_bar, P_hat, w, verbose)
    for r in 1:m    # For each machine r
        for g in 1:(max_gamma + 1)  # For each gamma value g
            for d in 1:(max_time_deviation + 1)     # For each accumulated proc time deviation
                ...
                C[r, 1, g, d] = deepcopy(C_mtx)
                dynmatrix[r, 1, g, d] = f!(pi, r, 1, C[r, 1, g, d], deviated_jobs[r, 1, g, d], m, n, P_bar, P_hat, w, verbose)
            end
        end
    end
    ...
    #C[1, 1, 1, Int64(ceil(ptk) + 1)] = zeros(Float64, m, n)
    dynmatrix[1, 1, 1, Int64(ceil(ptk) + 1)] = f!(pi, 1, 1, C[1, 1, 1, Int64(ceil(ptk) + 1)], deviated_jobs[1, 1, 1, Int64(ceil(ptk) + 1)], m, n, P_bar, P_hat, w, verbose)
    for g in 2:(gamma + 1)
        deviated_jobs[1, 1, g, Int64(ceil(ptk)) + 1] = deepcopy([ [] for i in 1:m ])
        deviated_jobs[1, 1, g, Int64(ceil(ptk)) + 1][1] = [ jobk ]
        dynmatrix[1, 1, g, Int64(ceil(ptk)) + 1], mtx = ff(pi, 1, 1, ptk, deviated_jobs[1, 1, g, Int64(ceil(ptk)) + 1], m, n, P_bar, P_hat, w, verbose)
        C[1, 1, g, Int64(ceil(ptk) + 1)] = mtx
        #dynmatrix[1, 1, g, Int64(ceil(ptk) + 1)] = f!(pi, 1, 1, C[1, 1, g, Int64(ceil(ptk) + 1)], deviated_jobs[1, 1, g, Int64(ceil(ptk) + 1)], m, n, P_bar, P_hat, w)
    ...
    C[1, 1, 1, 1] = zeros(Float64, m, n)
    dynmatrix[1, 1, 1, 1], mtx = ff(pi, 1, 1, 0, deviated_jobs[1, 1, 1, 1], m, n, P_bar, P_hat, w, verbose) # f!(pi, 1, 1, C[1, 1, 1, 1], deviated_jobs[1, 1, 1, 1], m, n, P_bar, P_hat, w)
    C[1, 1, 1, 1] = deepcopy(mtx)
    temp, mtx = ff(pi, 1, 1, 0, deviated_jobs, m, n, P_bar, P_hat, w, verbose)
    @assert temp == dynmatrix[1, 1, 1, 1]
    for k in 2:n
        deviated_jobs[1, k, 1, 1] = deepcopy(deviated_jobs[1, k - 1, 1, 1])
        C[1, k, 1, 1] = deepcopy(C[1, k - 1, 1, 1])
        dynmatrix[1, k, 1, 1] = f!(pi, 1, k, C[1, k, 1, 1], deviated_jobs[1, k, 1, 1], m, n, P_bar, P_hat, w, verbose)
        temp, mtx = ff(pi, 1, k, 0, deviated_jobs[1, k, 1, 1], m, n, P_bar, P_hat, w, verbose) # + dynmatrix[1, k - 1, 1, 1]
        #println("temp = $(temp) == dyn = $(dynmatrix[1, k, 1, 1])")
        @assert temp == dynmatrix[1, k, 1, 1]
        ...
        for d in 2:(max_time_deviation + 1)
            ...
            C[1, k, 1, d] = deepcopy(C[1, k, 1, d - 1])
            dynmatrix[1, k, 1, d] = f!(pi, 1, k, C[1, k, 1, d], deviated_jobs[1, k, 1, d], m, n, P_bar, P_hat, w, verbose)
            temp, mtx = ff(pi, 1, k, 0, deviated_jobs[1, k, 1, d], m, n, P_bar, P_hat, w, verbose)   #  -Inf
            #println("temp = $(temp) == dyn = $(dynmatrix[1, k, 1, d])")
            @assert temp == dynmatrix[1, k, 1, d]
        ...
        ptk = P_hat[1, jobk]
        for g in 2:(gamma + 1)
            deviated_jobs[1, k, g, Int64(ceil(ptk)) + 1] = deepcopy([ [] for i in 1:m ])
            deviated_jobs[1, k, g, Int64(ceil(ptk)) + 1][1] = [ jobk ]
            dynmatrix[1, k, g, Int64(ceil(ptk)) + 1], mtx = ff(pi, 1, 1, ptk, deviated_jobs[1, k, g, Int64(ceil(ptk)) + 1], m, n, P_bar, P_hat, w, verbose)
            C[1, k, g, Int64(ceil(ptk) + 1)] = mtx
            #dynmatrix[1, 1, g, Int64(ceil(ptk) + 1)] = f!(pi, 1, 1, C[1, 1, g, Int64(ceil(ptk) + 1)], deviated_jobs[1, 1, g, Int64(ceil(ptk) + 1)], m, n, P_bar, P_hat, w)
            #println("dynmatrix[1, 1, $(g), $(Int64(ceil(ptk)) + 1)] =  $(dynmatrix[1, 1, g, Int64(ceil(ptk)) + 1]) : deviated = $(deviated_jobs[1, 1, g, Int64(ceil(ptk)) + 1])")
        end
    end
    ...
    for r in 1:m
        ...
        if r == 1
            max_delayed_jobs_all_combinations = [ [ [] for i in 1:m ] ]
        #elseif r == 2
        ...
        for max_delayed_jobs_prev_machine in max_delayed_jobs_all_combinations
            if r >= 2
                ...
                deviated_jobs[r, 1, 1, 1] = deepcopy(max_delayed_jobs_prev_machine)  # deepcopy([ [] for i in 1:m ])
                C[r, 1, 1, 1] = deepcopy(max_C_prev_machine)
                dynmatrix[r, 1, 1, 1] = f!(pi, r, 1, C[r, 1, 1, 1], deviated_jobs[r, 1, 1, 1], m, n, P_bar, P_hat, w, verbose)
                temp, mtx = ff(pi, r, 1, 0, deviated_jobs[r, 1, 1, 1], m, n, P_bar, P_hat, w, verbose)
                @assert temp == dynmatrix[r, 1, 1, 1]
                # calculate processing time of job in position 1
                jobk = pi[1]
                ptk = P_hat[r, jobk]
                deviated_jobs[r, 1, 1, Int64(ceil(ptk)) + 1] = deepcopy(max_delayed_jobs_prev_machine)  # deepcopy([ [] for i in 1:m ])
                C[r, 1, 1, Int64(ceil(ptk)) + 1] = deepcopy(max_C_prev_machine)
                dynmatrix[r, 1, 1, Int64(ceil(ptk)) + 1] = f!(pi, r, 1, C[r, 1, 1, Int64(ceil(ptk)) + 1], deviated_jobs[r, 1, 1, Int64(ceil(ptk)) + 1], m, n, P_bar, P_hat, w, verbose)
                temp, mtx = ff(pi, r, 1, 0, deviated_jobs[r, 1, 1, Int64(ceil(ptk)) + 1], m, n, P_bar, P_hat, w, verbose)
                @assert temp == dynmatrix[r, 1, 1, Int64(ceil(ptk)) + 1]
                for g in 1:(gamma + 1)
                    for d in 1:(max_time_deviation+1)
                        C[r, 1, g, d] = deepcopy(max_C_prev_machine)
                        deviated_jobs[r, 1, g, d] = deepcopy(max_delayed_jobs_prev_machine)
                    end
                end
                for g in 2:(gamma + 1)
                    ...
                    C[r, 1, g, Int64(ceil(ptk)) + 1] = deepcopy(max_C_prev_machine)
                    dynmatrix[r, 1, g, Int64(ceil(ptk)) + 1] = f!(pi, r, 1, C[r, 1, g, Int64(ceil(ptk)) + 1], deviated_jobs[r, 1, g, Int64(ceil(ptk)) + 1], m, n, P_bar, P_hat, w, verbose)
                    temp, mtx = ff(pi, r, 1, ptk, deviated_jobs[r, 1, g, Int64(ceil(ptk)) + 1], m, n, P_bar, P_hat, w, verbose)
                    @assert temp == dynmatrix[r, 1, g, Int64(ceil(ptk)) + 1]
                ...
                for k in 2:n
                    ...
                    C[r, k, 1, 1] = deepcopy(C[r, k - 1, 1, 1])
                    dynmatrix[r, k, 1, 1] = f!(pi, r, k, C[r, k, 1, 1], deviated_jobs[r, k, 1, 1], m, n, P_bar, P_hat, w, verbose)
                    temp, mtx = ff(pi, r, k, 0, deviated_jobs[r, k, 1, 1], m, n, P_bar, P_hat, w, verbose) # + dynmatrix[r, k - 1, 1, 1]
                    @assert temp == dynmatrix[r, k, 1, 1]
                    for d in 2:(max_time_deviation + 1)
                        ...
                        C[r, k, 1, d] = deepcopy(max_C_prev_machine) #  deepcopy(C[r - 1, k, 1, d])
                        dynmatrix[r, k, 1, d] = f!(pi, r, k, C[r, k, 1, d], deviated_jobs[r, k, 1, d], m, n, P_bar, P_hat, w, verbose)
                        temp, mtx = ff(pi, r, k, 0, deviated_jobs[r, k, 1, d], m, n, P_bar, P_hat, w, verbose) # + dynmatrix[r, k - 1, 1, 1]  # dynmatrix[r - 1, k, 1, d]
                        @assert temp == dynmatrix[r, k, 1, d]
                    end
                end
            end
            for k in 2:n
                ...
                for g in 2:(gamma + 1)
                    ...
                    for d in 1:(max_time_deviation+1)
                        ...
                        if d - ptk - 1 < 0
                            deviated_jobs[r, k, g, d] = deepcopy(deviated_jobs[r, k - 1, g, d])
                            C[r, k, g, d] = deepcopy(C[r, k - 1, g, d])
                            dynmatrix[r, k, g, d] = f!(pi, r, k, C[r, k, g, d], deviated_jobs[r, k, g, d], m, n, P_bar, P_hat, w, verbose)
                            temp, mtx = ff(pi, r, k, d - 1, deviated_jobs[r, k, g, d], m, n, P_bar, P_hat, w, verbose) #+ dynmatrix[r, k - 1, g, d]
                            if verbose
                                println("r = $(r), k = $(k), g = $(g), d = $(d): deviated_jobs = $(deviated_jobs[r, k, g, d]); temp = $(temp) ; dyn = $(dynmatrix[r, k, g, d])")
                            end
                            @assert temp == dynmatrix[r, k, g, d]
                        else
                            #println("jobk = $(jobk) ; deviated_jobs[r, k - 1, g - 1, d - ptk] = $(deviated_jobs[r, k - 1, g - 1, d - ptk])")
                                ...
                                C[r, k, g, d] = deepcopy(C[r, k - 1, g - 1, d - ptk])
                                dynmatrix[r, k, g, d] = f!(pi, r, k, C[r, k, g, d], deviated_jobs[r, k, g, d], m, n, P_bar, P_hat, w, verbose)
                                temp, mtx = ff(pi, r, k, d, deviated_jobs[r, k, g, d], m, n, P_bar, P_hat, w, verbose) #+ dynmatrix[r, k - 1, g - 1, d - ptk]
                                if verbose
                                    println("r = $(r), k = $(k), g = $(g), d = $(d): deviated_jobs = $(deviated_jobs[r, k, g, d]); temp = $(temp) ; dyn = $(dynmatrix[r, k, g, d])")
                                end
                                @assert temp == dynmatrix[r, k, g, d]
        ...
        for d in reverse(1:max_time_deviation+1)
            ...
            if size(deviated_jobs[r, n, Int64(ceil(Γ[r])) + 1, d][r], 1) >= Int64(ceil(Γ[r]))
                #println("v = $(deviated_jobs[r, n, Int64(ceil(Γ[r])) + 1, d][r]); size = $(size(deviated_jobs[r, n, Int64(ceil(Γ[r])) + 1, d][r], 1))")
                if max_wct[r] < dynmatrix[r, n, Int64(ceil(Γ[r])) + 1, d]
                    ...
                    max_C_prev_machine = deepcopy(C[r, n, Int64(ceil(Γ[r])) + 1, d])
    ...
    for worst_Γ_scenario in max_delayed_jobs_all[m]
        worst_Γ_scenario_dev = calculate_deviation_matrix(m, n, pi, Γ, worst_Γ_scenario, [], "machine")
        wct = calculate_wct_given_scenario(pi, m, n, P_bar, P_hat, w, worst_Γ_scenario_dev, false )     # true)
        fits = true
        for i in 1:m
...
# P_hat             : the processing time deviations matrix
# w                 : the weight array for the weighted completion time calculation
function ff(pi, r, sequence_pos, current_deviation, deviated_jobs, m, n, P_bar, P_hat, w, verbose = false)
...
function test_m_machine_budget_worst_case_dp(m, n, P_bar, P_hat, pi, time_limit = 1800)
    ...
    for Γ1 in 4:n
        for Γ2 in 1:n
            for Γ3 in 3:n
                Γ = [Γ1, Γ2, Γ3]
                wct_brute, best_Γ, best_j = worst_case_wct_brute_force_3_machines(pi, 3, n, Γ, P_bar, P_hat, w, true)
                wct_dp, best_Γ_2 = worst_case_wct_dp_m_machines(pi, 3, n, Γ, P_bar, P_hat, w)  #, false)
...
function test_m_machine_budget_worst_case_dp_10jobs_2machines()
    ...
    basedir = joinpath(robust_ying_instances_folder, "/data/10jobs")
    error_count = 0
    file_count = 0
    filename_list = []
    error_count_list = []
...
function test_m_machine_budget_worst_case_dp_2machines_specific()
    searchdir(path,key) = filter(x->occursin(key,x), readdir(path))
    basedir = joinpath(robust_ying_instances_folder, "rob-pfsp-wct/10jobs")
    for filename in searchdir(basedir, ".txt")
        if filename == "RB0103001_10_2_30_wct_inputs.txt"
            inputfile = "$(basedir)/$(filename)"
            #m, n, P_bar, P_hat = read_robust_ying_input_file(inputfile)
            ...
            for Γ1 in 3:n
                for Γ2 in 1:n
                    for Γ3 in 1:n
                        ...
                        wct_dp, worst_Γ_scenario_dp = worst_case_wct_dp_m_machines(pi, m, n, [Γ1, Γ2, Γ3], P_bar, P_hat, w)
...
test_m_machine_budget_worst_case_dp_2machines_specific()  # 29 errors, 16,5s, 3,5GB
