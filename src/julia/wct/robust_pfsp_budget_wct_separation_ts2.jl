# ===================================================================================================
# robust_pfsp_budget_wct_separation_wagner_ts2.jl
...
# based on TS2 PFSP MILP Model.
# Reference for TS2 PFSP Model:
...
function separate_ts2(model, m, n, Γ, P_bar, P_hat, w, iteration, UB, best_pi, bigM; max_cores = 16,
    ...
    for permutation in Set(permutation_current)  # Process all initial solutions (or current CCG solution)
        ...
        if gap > EPS  # If Gap = (UB - LB) / LB <= EPS, an eps-optimal solution is found, terminate.
            ...
            for z_sep in z_sep_list
                ...
                newE = @variable(model, [M, N], base_name = "newE_$(iteration)-$(count)", lower_bound = 0)
                ...
                for r in 2:m  # (A.17)
                    for j in 1:(n-1)
                        @constraint(model, newE[r, j+1] >= newE[r, j] + sum((P_bar[r, i] + P_hat[r, i]*z_sep[r, i]) * Z[i, j+1] for i in Jobs))
                    end
                end
                for r in 1:(m-1)  # (A.18)
                    for j in 2:n
                        @constraint(model, newE[r+1, j] >= newE[r, j] + sum((P_bar[r+1, i] + P_hat[r+1, i]*z_sep[r+1, i]) * Z[i, j] for i in Jobs))
                    end
                end
                for j in 1:(n-1)  # (A.19)
                    @constraint(model, newE[1, j+1] == newE[1, j] + sum((P_bar[1, i] + P_hat[1, i]*z_sep[1, i]) * Z[i, j+1] for i in Jobs))
                end
                for r in 1:(m-1)  # (A.20)
                    @constraint(model, newE[r+1, 1] == newE[r, 1] + sum((P_bar[r+1, i] + P_hat[r+1, i]*z_sep[r+1, i]) * Z[i, 1] for i in Jobs))
                end
                # (A.21)
                @constraint(model, newE[1, 1] == sum((P_bar[1, i] + P_hat[1, i]*z_sep[1, i]) * Z[i, 1] for i in Jobs))
                for i in 1:n  # Disjuctive constraints used to determine the start time of job i on the last machine m
                    for j in 1:n
                        @constraint(model, newF[i] >= newE[m, j] - bigM * (1 - Z[i, j]) )
                    end
                end
            end
            total_count += count
...
# ===================================================================================
function solve_robust_model_flowshop_wct_budget_separation_ts2(m, n, P_bar, P_hat, w, Γ, time_limit; mp_bigM_type = 2, sp_bigM_type = 3, max_cores = 16,
    ...
    bigM = max_cmax  # Big-M
    println("Starting TS2 C&CG procedure...")
    ...
    # # E_ik completion time of job k on machine i
    E = @variable(model, [M, N], base_name = "newE_1", lower_bound = 0)
    # F[i] : completion time of job i on the last machine (M_m)
    ...
    for i in 1:n  # Disjuctive constraints used to determine the start time of job i on the last machine m
        for j in 1:n
            @constraint(model, F[i] >= E[m, j] - bigM * (1 - Z[i, j]) )
        end
    end
    ...
    while true
        # Solve the problem
        println("====================================================================\n")
        println("Solving WCT budget robust TS2 model C & CG (Iteration $(iter))...")
        println("====================================================================\n")
        ...
        if status == MOI.OPTIMAL
            ...
            if warm_start
                ...
                println("\n===========  T S 2    R O B U S T    S O L U T I O N  ===========\n")
            ...
            separation_done, mip_optimal, UB, LB, best_pi, gap, sp_time, cut_count = separate_ts2(model, m, n, Γ, P_bar, P_hat, w, iter + 1, UB, best_pi, bigM;
